...
<script>
	...
	function changeType(){
		$("#_type").val($("#_typeId option:selected").text().substring($("#_typeId option:selected").text().indexOf("_")+1));
	}
</script>
<div class="panel panel-default div_screen">
			<form id="searchForm"
				action="${pageContext.request.contextPath}/sys/project/hisPage"
				method="get">
				...
				<div class="panel-heading">
					<table style="width: 100%;" id="searchTable">
						<tr>
							<td style="width: 25%;">项目名称: <input name="search_LIKE_cname"
								value="${search_LIKE_cname }" class="input-control"></td>
							<td align="right">
...
<div id="initStaffsHis" class="panel panel-default div_screen">
	...
	<table style="width: 100%;" frame="below" bordercolorlight="#f5f5f5"
		...
		<tr bgcolor="#f5f5f5" height="35px">
			<td width="92%" style="padding-left: 10px">项目列表
			</td>
			<td>
				<button id="_newBtn" class="btn btn-primary btn-sm" url="${pageContext.request.contextPath}/sys/project/new" modal="modal">
	...
	<table
		...
		<thead>
			<tr>
				<th class="table_screentitle">项目中文名称</th>
				<th class="table_screentitle">项目ID</th>
				<th class="table_screentitle">项目类型</th>
		...
		<c:forEach items="${pageHis.content}" var="t">
			<tr align="center">
				<td><a name='_showBtn' href="javascript:void(0)" url="${pageContext.request.contextPath}/sys/project/${t.id}">${t.cname}</a></td>
				<td>${t.no}</td>
				<td>${t.typeId}_${t.type}</td>
				<td>
					<c:choose>
						<c:when test="${t.flag==1}"> 启用 </c:when>
						<c:when test="${t.flag==0}"> 停用</c:when>
					</c:choose></td>
				<td><a name="_upBtn" href="javascript:void(0)" url="${pageContext.request.contextPath}/sys/project/edit/${t.id}">修改
				</a> || <a href="javascript:void(0)"
					onclick="del('${pageContext.request.contextPath}/sys/project/changeFlag?id=${t.id}&flag=${t.flag}')">
					<c:choose>
