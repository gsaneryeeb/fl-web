...
<div class="panel panel-default div_screen">
			<form id="searchForm"
				action="${pageContext.request.contextPath}/sys/part/hisPage"
				method="get">
...
<div id="initStaffsHis" class="panel panel-default div_screen">
	...
	<table style="width: 100%;" frame="below" bordercolorlight="#f5f5f5"
		...
		<tr bgcolor="#f5f5f5" height="35px">
			...
			<td>
				<button id="_newBtn" class="btn btn-primary btn-sm" url="${pageContext.request.contextPath}/sys/part/new" modal="modal">
	...
	<table
		...
		<c:forEach items="${pageHis.content}" var="t">
			<tr align="center">
				<td><a name='_showBtn' href="javascript:void(0)" url="${pageContext.request.contextPath}/sys/part/${t.id}">${t.no}</a></td>
				<td>${t.model}</td>
				<td>${t.name}</td>
				<td>${t.partDes}</td>
				<td>${t.partFru}</td>
				...
				<td><a name="_upBtn" href="javascript:void(0)" url="${pageContext.request.contextPath}/sys/part/edit/${t.id}">修改
				</a> || <a href="javascript:void(0)"
					onclick="del('${pageContext.request.contextPath}/sys/part/changeFlag?id=${t.id}&flag=${t.flag}')">
					<c:choose>
