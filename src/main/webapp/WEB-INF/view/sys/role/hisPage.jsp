...
function add(url) {
	$("#_curdDiv").load(url, function() {
		...
		$("#tree").load("${pageContext.request.contextPath}/sys/role/getRoles",function(msg){
...
function checkroleSave(method) {
	...
	function update(url,id) {
		$("#_curdDiv").load(url, function() {
			...
			$("#tree").load("${pageContext.request.contextPath}/sys/role/getRoles?id="+id,function(msg){
				$.fn.zTree.init($("#tree"), setting, eval(msg));
			});
		});
	}
...
<div class="panel panel-default div_screen">
			<form id="searchForm"
				action="${pageContext.request.contextPath}/sys/role/hisPage"
				method="get">
				...
				<div class="panel-heading">
					<table style="width: 100%;" id="searchTable">
						<tr>
							<td style="width: 25%;">角色名称: <input name="search_LIKE_cname"
								id="s_userName" value="${search_LIKE_cname }" class="input-control"></td>
							<td align="right">
...
<div id="initStaffsHis" class="panel panel-default div_screen">
	...
	<table style="width: 100%;" frame="below" bordercolorlight="#f5f5f5"
		...
		<tr bgcolor="#f5f5f5" height="35px">
			<td width="92%" style="padding-left: 10px">角色列表</td>
			<td>
				<button class="btn btn-primary btn-sm"
					onclick="add('${pageContext.request.contextPath}/sys/role/new')">
	...
	<table
		...
		<thead>
			<tr>
				<th class="table_screentitle">角色中文名</th>
				<th class="table_screentitle">角色英文名</th>
		...
		<c:forEach items="${pageHis.content}" var="t">
			<tr align="center">
				<td><a href="javascript:void(0)" onclick="show('${pageContext.request.contextPath}/sys/role/${t.id}','${t.id }')">${t.cname}</a></td>
				<td>${t.ename}</td>
				<td>
					<c:choose>
						<c:when test="${t.availability==1}"> 启用 </c:when>
						<c:when test="${t.availability==0}"> 停用</c:when>
					</c:choose></td>
				<td>
				<a href="javascript:void(0)"
					onclick="update('${pageContext.request.contextPath}/sys/role/edit?id=${t.id}','${t.id }')">修改</a> 
					|| <a href="javascript:void(0)"
					onclick="del('${pageContext.request.contextPath}/sys/role/changeFlag?id=${t.id}&availability=${t.availability}')">
					<c:choose>
						<c:when test="${t.availability==0}"> 启用 </c:when>
						<c:when test="${t.availability==1}"> 停用</c:when>
					</c:choose>
					</a></td>
			</tr>
		</c:forEach>
