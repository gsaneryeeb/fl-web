...
<div class="panel panel-default div_screen">
			<form id="searchForm"
				action="${pageContext.request.contextPath}/sys/station/hisPage"
				method="get">
				...
				<div class="panel-heading">
					<table style="width: 100%;" id="searchTable">
						<tr>
							<td style="width: 25%;">服务站名称: <input name="search_LIKE_name"
...
<div id="initStaffsHis" class="panel panel-default div_screen">
	...
	<table style="width: 100%;" frame="below" bordercolorlight="#f5f5f5"
		...
		<tr bgcolor="#f5f5f5" height="35px">
			<td width="92%" style="padding-left: 10px">服务站列表
			</td>
			<td>
				<button id="_newBtn" class="btn btn-primary btn-sm" url="${pageContext.request.contextPath}/sys/station/new" modal="modal">
	...
	<table
		...
		<thead>
			<tr>
				<th class="table_screentitle">服务站编号</th>
				<th class="table_screentitle">服务站名称</th>
				<th class="table_screentitle">项目</th>
		...
		<c:forEach items="${pageHis.content}" var="t">
			<tr align="center">
				<td><a name='_showBtn' href="javascript:void(0)" url="${pageContext.request.contextPath}/sys/station/${t.id}">${t.no}</a></td>
				<td>${t.name}</td>
				<td>${t.project.no}</td>
				<td>
					<c:choose>
				...
				<td><a name="_upBtn" href="javascript:void(0)" url="${pageContext.request.contextPath}/sys/station/edit/${t.id}">修改
				</a> || <a href="javascript:void(0)"
					onclick="del('${pageContext.request.contextPath}/sys/station/changeFlag?id=${t.id}&flag=${t.flag}')">
					<c:choose>
