...
<script>
	...
	function checkSave() {
		var bl=true;
		if($("#_evaluationProject").val()==""){
			alert("评分项目为必填");
			bl=false;
			$("#_evaluationProject").focus();
			return bl;
		}
		if($("#_content").val()==""){
			alert("评分内容为必填");
...
<div class="panel panel-default div_screen">
			<form id="searchForm"
				action="${pageContext.request.contextPath}/sys/evaluation/hisPage"
				method="get">
				...
				<div class="panel-heading">
					<table style="width: 100%;" id="searchTable">
						<tr>
							<td style="width: 25%;">评分项目: <input name="search_LIKE_evaluationProject" value="${search_LIKE_evaluationProject }" class="input-control"></td>
							<td align="right">
...
<div id="initStaffsHis" class="panel panel-default div_screen">
	...
	<table style="width: 100%;" frame="below" bordercolorlight="#f5f5f5"
		...
		<tr bgcolor="#f5f5f5" height="35px">
			<td width="92%" style="padding-left: 10px">服务评分列表
			</td>
			<td>
				<button id="_newBtn" class="btn btn-primary btn-sm" url="${pageContext.request.contextPath}/sys/evaluation/new" modal="modal">
	...
	<table
		...
		<thead>
			<tr>
				<th class="table_screentitle">评分项目</th>
				<th class="table_screentitle">评分内容</th>
				<th class="table_screentitle">评分</th>
		...
		<c:forEach items="${pageHis.content}" var="t">
			<tr align="center">
				<td><a name='_showBtn' href="javascript:void(0)" url="${pageContext.request.contextPath}/sys/evaluation/${t.id}">${t.evaluationProject}</a></td>
				<td>${t.content}</td>
				<td>${t.evaluation}</td>
				<td>
					<c:choose>
				...
				<td><a name="_upBtn" href="javascript:void(0)" url="${pageContext.request.contextPath}/sys/evaluation/edit/${t.id}">修改
				</a> || <a href="javascript:void(0)"
					onclick="del('${pageContext.request.contextPath}/sys/evaluation/changeFlag?id=${t.id}&flag=${t.flag}')">
					<c:choose>
