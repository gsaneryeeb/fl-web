						...
						<table class="table	table-bordered table-condensed" style="width: 100%;">
							<tbody>
								...
								<tr>
									...
									<td align="left">
									<select name="allocateEngineerId" id="_allocateEngineer">
										<option value=""></option>
										<c:forEach items="${engineers }" var="e">
											<option value="${e.id }" <c:if test='${e.id==t.allocateEngineerId }'> selected="selected"</c:if> dir="${e.uName }">${e.userNo }_${e.uName }</option>
										</c:forEach>
									</select>
									</td>
								</tr>
