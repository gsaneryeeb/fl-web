							...
							<tbody>
								...
								<tr ><!-- id="i4" -->
									<td colspan="4" class="warning" style="text-align: left;">
										注意事项:<br>
										&nbsp;&nbsp;&nbsp;&nbsp;对于计算机类产品，用户在维修前应做好数据的备份，用户应对其数据的安全性自行负责，产品服务机构不承担数据、程序或可移动存储介质的损坏或丢失的责任。如客户单方面放弃维修，需支付检测费。用户应在收到取机通知三个月内到本服务中心取机，逾期不取的我方不对维修产品的毁损灭失风险承担任何责任。请确认您已经了解在维修过程中您的机器内的现有数据和程序可能会损坏或丢失，已经认真阅读和完全了解并同意以上声明条款内容。
									</td>
								</tr>
								<tr>
								...
								<tr id="i1">
									<td colspan="4" class="success" align="left">故障类型: <input type="checkbox" name="_faultType" value="SOFT" <c:if test='${t.faultType=="SOFT"}'> checked="checked"</c:if>>软件服务 <input type="checkbox" name="_faultType" value="HARD" <c:if test='${t.faultType=="HARD"}'> checked="checked"</c:if>> 硬件服务<input type="hidden" name="faultType" value="${t.faultType }" id="faultType"> <input type="checkbox" name="_faultType" value="SENCEND_REPER" <c:if test='${t.faultType=="SENCEND_REPER"}'> checked="checked"</c:if>>二级维修 </td>
								</tr>
								<tr id="i2">
